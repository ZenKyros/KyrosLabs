...
import { Search, Sun, Moon, Menu, X, ArrowUpRight, Github, Linkedin } from "lucide-react";
...
export default function Layout() {
  ...
  // page title + scroll restore per route (pathname only — query-param
  // changes like /graph?focus=… should not reset scroll position)
  useEffect(() => {
        ...
        <div className="max-w-6xl mx-auto px-5 py-14 grid md:grid-cols-[1.4fr_1fr_1fr_1.2fr] gap-10">
          <div>
            ...
            <p className="text-[13.5px] text-muted leading-relaxed mt-4 max-w-[30ch]">
              A curated collection of AI concepts, insights, notes, and key learnings.
            </p>
          ...
          <div>
            <div className="text-[11px] font-semibold tracking-[0.12em] uppercase text-faint mb-4">Recent notes</div>
            <ul className="space-y-2.5">
              {vault.recent.slice(0, 5).map((note) => (
                <li key={note.slug}>
                  <Link to={`/note/${note.slug}`} className="block truncate text-[13.5px] text-muted hover:text-accent transition-colors">
                    {note.meta.title}
                  </Link>
                </li>
              ))}
          ...
          <div>
            <div className="text-[11px] font-semibold tracking-[0.12em] uppercase text-faint mb-4">Contact</div>
            <p className="text-[13px] text-muted leading-relaxed max-w-[28ch]">
              Follow along as I build AI systems, study new ideas, and turn notes into projects.
            </p>
            <div className="flex flex-col items-start gap-3 mt-5">
              <a
                ...
                className="group inline-flex items-center gap-2.5 text-[13.5px] text-muted hover:text-accent transition-colors"
              >
                <Github size={15} />
                GitHub

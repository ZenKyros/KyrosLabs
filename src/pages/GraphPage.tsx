...
import { buildFileSystemGraph, getFileSystemNeighborhood, type FileSystemGraphData } from "../engine/filesystem-graph";
...
export default function GraphPage() {
  ...
  const legacyData: GraphData = useMemo(() => {
    if (useFileSystemGraph) return { nodes: [], edges: [] };
          ...
          <div>
            ...
            {focusNode.metadata?.description && (
              <p className="font-serif2 italic text-muted text-[15.5px] leading-relaxed mt-3 max-w-xl">
                {String(focusNode.metadata.description)}
              </p>
            )}
          </div>
